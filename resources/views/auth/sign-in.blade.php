...
@section('content')
    <div class="grid lg:grid-cols-2 grow">
                ...
                <form action="{{ route('login') }}" method="POST" class="kt-card-content flex flex-col gap-5 p-10"
                    ...
                    @if (session('status') === 'locked')
                        <div class="kt-alert kt-alert-warning flex items-center gap-2" role="alert">
                            ...
                            <span
                                class="text-sm">{{ __('Your account is locked. Please contact an administrator.') }}</span>
                        </div>
                    @endif
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <input id="email" name="email" type="email"
                            class="kt-input @error('email') border-danger @enderror" placeholder="email@example.com"
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <div class="flex items-center gap-5">
                            <label class="kt-label flex items-center gap-2 cursor-pointer">
                                <input class="kt-radio kt-radio-sm" type="radio" name="app_target" value="admin"
                                    {{ old('app_target', session('last_app', 'admin')) === 'admin' ? 'checked' : '' }} />
                                <span class="text-sm text-mono">Admin</span>
                            </label>
                            <label class="kt-label flex items-center gap-2 cursor-pointer">
                                <input class="kt-radio kt-radio-sm" type="radio" name="app_target" value="crm"
        ...
        {{-- Branded hero column --}}
        <div
            class="lg:rounded-xl lg:border lg:border-border lg:m-5 order-1 lg:order-2 bg-top xl:bg-cover bg-no-repeat branded-bg">
            <div class="flex flex-col p-8 lg:p-16 gap-4">
                <img id="logoHero" class="h-8" />
                <div class="flex flex-col gap-3 mt-auto">
                    <h3 class="text-2xl font-semibold text-muted">Secure Operations Portal</h3>
                    <p class="text-base font-medium text-muted">
                        A robust authentication gateway ensuring<br />
                        secure, <span class="text-muted font-semibold">efficient access</span> to the Carsmart<br />
                        internal dashboard.
                    </p>
                </div>
            </div>
