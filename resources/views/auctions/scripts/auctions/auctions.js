...
(function () {
    ...
    function toast(msg, type = "success") {
        ...
        n.className =
            "px-3 py-2 rounded text-sm shadow text-white pointer-events-auto " +
            ({
                error: "bg-red-600",
    ...
    const auctions = [
        {
            ...
            start: "2025-10-12T10:00",
            end: "2025-10-12T16:00",
            status: "Planned",
            visibility: "Public",
            owner: "JR",
            description: "Monthly flagship sale of quality vehicles.",
            cohort_tag: "Prime",
            closing_style: "single",
            proxy_bidding: true,
            sniper_protection: true,
            sniper_minutes: 2,
            auto_accept: true,
            bin_precedence: true,
            start_price_mode: "zero",
            increment_schema: "standard",
            ...
            activity: [
                {
                    ts: "2025-10-01T09:00Z",
                    actor: "JR",
                    event: "auction_created",
                    detail: "Auction created as Draft",
                },
                {
                    ts: "2025-10-05T14:00Z",
                    actor: "JR",
                    event: "auction_published",
                    detail: "Auction published",
                },
            ],
            exchange_proposals: [
                {
                    id: "EP-001",
                    lot_id: "L001",
                    offered_by: "Vendor Y Cars",
                    offered_vehicle: "Mercedes C220d 2020 (LST-1041)",
                    cash_diff: 500,
                    expiry: "2025-10-12T09:00",
                    notes: "Clean car, full service history",
                    active: true,
                },
            ],
        },
        {
            id: "AUC-206",
            name: "Evening Prestige",
            start: "2025-10-20T18:00",
            end: "2025-10-20T21:00",
            status: "Published",
            visibility: "Private",
            owner: "AM",
            description: "Curated prestige and performance vehicles.",
            cohort_tag: "Prestige",
            closing_style: "staggered",
            proxy_bidding: true,
            sniper_protection: true,
            sniper_minutes: 3,
            auto_accept: true,
            bin_precedence: true,
            start_price_mode: "guide",
            increment_schema: "premium",
            participant_mode: "invite",
            gate_kyc: true,
            gate_card: true,
            ...
            lots: [
                {
                    ...
                    lot_num: "001",
                    listing: "LST-1055",
                    vehicle: "Porsche 911 Carrera 2021",
                    start_price: 0,
                    reserve: true,
                    bin_enabled: false,
                    state: "Ready",
            ...
            activity: [
                {
                    ts: "2025-10-08T10:00Z",
                    actor: "AM",
                    event: "auction_created",
                    detail: "Auction created",
                },
                {
                    ts: "2025-10-09T11:00Z",
                    actor: "AM",
                    event: "auction_published",
                    detail: "Published",
                },
    ...
    function statusBadge(s) {
        const map = {
            Planned: "kt-badge-outline",
            Published: "kt-badge-info",
            Live: "kt-badge-success",
            Paused: "kt-badge-warning",
            Ended: "kt-badge-secondary",
    ...
    function getFiltered() {
        return auctions.filter((a) => {
            ...
            if (
                t &&
                ![a.id, a.name, a.owner].some((v) =>
                    v?.toLowerCase().includes(t),
                )
            )
                return false;
            if (state.filterStatus && a.status !== state.filterStatus)
                return false;
            if (
                state.filterVisibility &&
                a.visibility !== state.filterVisibility
            )
                return false;
            return true;
        });
    ...
    function renderTable() {
        ...
        tbody.innerHTML = filtered
            .map((a) => {
                const liveLots = a.lots.filter(
                    (l) => l.state === "Live",
                ).length;
                const endedLots = a.lots.filter(
                    (l) => l.state === "Ended",
                ).length;
        ...
        // checkboxes
        tbody.querySelectorAll(".auction-cb").forEach(
            (cb) =>
                (cb.onchange = () => {
                    cb.checked
                        ? state.selectedAuctions.add(cb.dataset.id)
                        : state.selectedAuctions.delete(cb.dataset.id);
                    updateBulkCount();
                }),
        );
        ...
        $("auctions-pagination").textContent =
    ...
    function updateBulkCount() {
        ...
        $("select-all").indeterminate =
            state.selectedAuctions.size > 0 &&
            state.selectedAuctions.size < auctions.length;
        $("select-all").checked =
            state.selectedAuctions.size === auctions.length &&
            auctions.length > 0;
    }
    ...
    function resetFilters() {
        ["filter-search", "filter-status", "filter-visibility"].forEach(
            (id) => $(id) && ($(id).value = ""),
        );
        state.filterSearch = "";
        state.filterStatus = "";
    ...
    function exportCSV() {
        ...
        const csv = [
            "ID,Name,Start,End,Lots,Status,Visibility,Owner",
            ...rows,
        ].join("\n");
    ...
    function bindViewSwitching() {
        document.querySelectorAll(".view-tab-btn").forEach(
            (b) =>
                (b.onclick = () => {
                    ...
                    $("view-list").classList.toggle(
                        "hidden",
                        state.view !== "list",
                    );
                    $("view-calendar").classList.toggle(
                        "hidden",
                        state.view !== "calendar",
                    );
    ...
    function bindSelectAll() {
        $("select-all").onchange = function () {
            ...
            if (this.checked)
                auctions.forEach((a) => state.selectedAuctions.add(a.id));
            updateBulkCount();
            renderTable();
        };
    }
        ...
        document.querySelectorAll("[data-bulk]").forEach(
            (b) =>
                (b.onclick = () => {
                    ...
                    if (!ids.length) {
                        toast("Select auctions first", "warning");
                        return;
                    }
                    ...
                    if (action === "duplicate")
                        ids.forEach((id) => duplicateAuction(id, true));
                    if (action === "archive")
                        ids.forEach((id) => {
                            const a = auctions.find((x) => x.id === id);
    ...
    function renderCalendar() {
        ...
        title.textContent = state.calDate.toLocaleString("en-GB", {
            month: "long",
            year: "numeric",
        });
        ...
        cells.forEach((d) => {
            if (!d) {
                html += `<div class="min-h-[80px] bg-muted/10 p-1"></div>`;
                return;
            }
            const isToday =
                d === today.getDate() &&
        ...
        grid.querySelectorAll('[data-act="open"]').forEach(
            (el) =>
                (el.onclick = (e) => {
                    e.stopPropagation();
                    openAuctionDetail(el.dataset.id);
                }),
        );

        // drag-create: clicking empty day opens create modal
        grid.querySelectorAll("[data-cal-day]").forEach(
            (cell) =>
                (cell.onclick = () => {
                    ...
                    if (!cell.querySelector("[data-id]")) {
                        ...
                        setTimeout(() => {
                            const startInput = document.querySelector(
                                '#create-auction-form [name="start"]',
                            );
    ...
    function bindCalendarControls() {
        ...
        document.querySelectorAll(".cal-view-btn").forEach(
            (b) =>
                (b.onclick = () => {
                    ...
                    document.querySelectorAll(".cal-view-btn").forEach((x) => {
                        x.classList.toggle(
                            "kt-btn-mono",
                            x.dataset.calView === state.calView,
                        );
                        x.classList.toggle(
                            "kt-btn-ghost",
                            x.dataset.calView !== state.calView,
    ...
    function initWizard() {
        ...
        document
            .querySelectorAll('[name="closing_style"]')
            .forEach(
                (r) =>
                    (r.onchange = () =>
                        $("staggered-interval-wrap").classList.toggle(
                            "hidden",
                            r.value !== "staggered",
                        )),
            );
        ...
        if (sniperToggle)
            sniperToggle.onchange = () =>
                $("sniper-minutes-wrap").classList.toggle(
                    "hidden",
                    !sniperToggle.checked,
                );

        // start price custom reveal
        document.querySelector('[name="start_price_mode"]').onchange =
            function () {
                $("start-price-custom-wrap").classList.toggle(
                    "hidden",
                    this.value !== "custom",
                );
            };
        ...
        document
            .querySelectorAll('[name="participant_mode"]')
            .forEach(
                (r) =>
                    (r.onchange = () =>
                        $("participant-set-select").classList.toggle(
                            "hidden",
                            r.value !== "set",
                        )),
            );
        ...
        // step pills click
        document
            .querySelectorAll(".wizard-pill")
            .forEach(
                (p) =>
                    (p.onclick = () =>
                        showWizardStep(parseInt(p.dataset.pill))),
            );
    }
    ...
    function showWizardStep(step) {
        ...
        document
            .querySelectorAll("#create-auction-form .wizard-step")
            .forEach((s) =>
        ...
        $("wizard-create").classList.toggle(
            "hidden",
            step !== state.wizardTotal,
        );
        ...
        $("wizard-step-label").textContent =
            `Step ${step} of ${state.wizardTotal}`;
        $("wizard-progress-bar").style.width =
            Math.round(((step - 1) / (state.wizardTotal - 1)) * 100) + "%";
    ...
    function advanceWizard() {
        ...
        if (state.wizardStep === 2) {
            ...
            if (!start || !end) {
                toast("Start and end date/time required", "error");
                return;
            }
    ...
    function generateWizardSummary() {
        ...
        fd.forEach((val, key) => {
            if (val && typeof val === "string")
                rows.push(`<div><strong>${key}:</strong> ${val}</div>`);
        });
        $("auction-wizard-summary").innerHTML =
    ...
    function createAuctionFromWizard(asDraft = false) {
        ...
        if (!name) {
            toast("Auction name is required", "error");
            return;
        }

        const start = fd.get("start"),
            end = fd.get("end");
        if (!asDraft && (!start || !end)) {
            toast("Schedule required before creating", "error");
            return;
        }
            ...
            activity: [
                {
                    ...
                    actor: "Me",
                    event: "auction_created",
                    detail: asDraft ? "Draft saved" : "Auction created",
                },
            ],
            exchange_proposals: [],
        };
    ...
    function populateAuctionDetail(a) {
        ...
        $("auc-detail-sub").textContent =
            `${a.status} · ${fmtDT(a.start)} → ${fmtDT(a.end)} · Owner: ${a.owner}`;

        // badges
        const badges = $("auc-detail-badges");
        badges.innerHTML = "";
        if (a.visibility === "Private")
            badges.innerHTML += `<span class="kt-badge kt-badge-sm kt-badge-outline">Private</span>`;
        if (a.sniper_protection)
            badges.innerHTML += `<span class="kt-badge kt-badge-sm kt-badge-info">Sniper ✓</span>`;
        if (a.proxy_bidding)
            badges.innerHTML += `<span class="kt-badge kt-badge-sm kt-badge-info">Proxy ✓</span>`;

        // KPIs
        $("auc-kpi-lots").textContent = a.lots.length;
        $("auc-kpi-live").textContent = a.lots.filter(
            (l) => l.state === "Live",
        ).length;
        $("auc-kpi-ended").textContent = a.lots.filter(
            (l) => l.state === "Ended",
        ).length;
        $("auc-kpi-reserve").textContent =
            `${a.lots.filter((l) => l.reserve_met).length}/${a.lots.length}`;
        $("auc-kpi-bidders").textContent = a.participants.filter(
            (p) => p.status === "Accepted",
        ).length;
        ...
        if (alerts) {
            ...
            if (!a.participants.length && a.participant_mode !== "all")
                warn.push("No participants invited");
            alerts.innerHTML = warn.length
                ? warn
                      .map((w) => `<div class="text-yellow-700">⚠ ${w}</div>`)
                      .join("")
                : '<div class="text-green-600">No alerts</div>';
        }

        if ($("overview-schedule"))
            $("overview-schedule").innerHTML =
                `${fmtDT(a.start)}<br><span class="text-muted-foreground">→ ${fmtDT(a.end)}</span>`;

        if ($("overview-rules"))
            $("overview-rules").innerHTML = `
        ...
        if ($("overview-participants"))
            $("overview-participants").textContent =
    ...
    function populateLots(a) {
        ...
        tbody.querySelectorAll("[data-lot-act]").forEach((b) => {
            b.onclick = () => {
                ...
                if (act === "withdraw") {
                    lot.state = "Withdrawn";
                    logEvent("lot_withdrawn", { lotId: lot.id });
                    toast(`Lot ${lot.lot_num} withdrawn`);
                    populateLots(a);
                }
                if (act === "rerun") {
                    toast(`Re-run scheduled for lot ${lot.lot_num}`, "info");
                    logEvent("lot_rerun_created", { lotId: lot.id });
                }
    ...
    function populateParticipantsTab(a) {
        ...
        tbody.innerHTML = a.participants
            .map(
                (p) => `
            <tr>
        ...
        tbody.querySelectorAll("[data-p-act]").forEach(
            (b) =>
                (b.onclick = () => {
                    if (b.dataset.pAct === "resend") {
                        toast(`Invite resent to ${b.dataset.pName}`);
                        logEvent("auction_invite_sent", {
                            vendor: b.dataset.pName,
                        });
                    }
                    if (b.dataset.pAct === "revoke") {
                        a.participants = a.participants.filter(
                            (p) => p.name !== b.dataset.pName,
                        );
                        toast(`${b.dataset.pName} revoked`);
                        populateParticipantsTab(a);
                    }
                }),
        );
    ...
    function populateActivityPanel(a) {
        ...
        el.innerHTML =
            ...
            <div class="flex gap-3 border-b border-border pb-2">
                ...
                .join("") ||
            '<div class="text-xs text-muted-foreground">No activity</div>';
    }
    ...
    function populateBidFeedFilter(a) {
        ...
        sel.innerHTML =
            ...
            a.lots
                .map(
                    (l) =>
                        `<option value="${l.id}">Lot ${l.lot_num} — ${l.vehicle}</option>`,
                )
                .join("");
    }
    ...
    function bindDetailActions() {
        ...
        $("auc-btn-start").onclick = () => {
            ...
            if (!a.lots.length) {
                toast("Cannot start: no lots", "error");
                return;
            }
            a.status = "Live";
            a.lots.forEach((l) => {
                if (l.state === "Ready") l.state = "Live";
            });
            logEvent("auction_started", { id: a.id });
            toast(`Auction ${a.id} started`);
            populateAuctionDetail(a);
            renderTable();
        };
        $("auc-btn-pause").onclick = () => {
            if (!state.activeAuction) return;
            state.activeAuction.status =
                state.activeAuction.status === "Paused" ? "Live" : "Paused";
            logEvent("auction_paused", { id: state.activeAuction.id });
            toast(`${state.activeAuction.id} ${state.activeAuction.status}`);
            populateAuctionDetail(state.activeAuction);
            renderTable();
        };
        ...
        $("auc-btn-add-lots").onclick = () => {
            if (!state.activeAuction) return;
            addMockLot(state.activeAuction);
        };
        $("auc-btn-invite").onclick = () => openModal($("participants-modal"));
        $("auc-btn-announce").onclick = () => {
            switchAucTab("messages");
        };
        $("auc-more-toggle").onclick = () =>
            $("auc-more-menu").classList.toggle("hidden");
        document.querySelectorAll("[data-auc-action]").forEach(
            (b) =>
                (b.onclick = () => {
        ...
        $("btn-send-announcement").onclick = () => {
            ...
            if (!txt || !state.activeAuction)
                return toast("Enter announcement text", "warning");
            state.activeAuction.messages.push({
                ts: new Date().toISOString(),
                text: txt,
            });
            ...
            logEvent("announcement_sent", {
                id: state.activeAuction.id,
                text: txt,
            });
            toast("Announcement sent");
        };

        $("btn-add-from-listings").onclick = () => {
            if (state.activeAuction) addMockLot(state.activeAuction);
        };
        $("btn-open-live-console").onclick = () => {
            if (state.activeAuction) openLiveConsole(state.activeAuction.id);
        };
    }
    ...
    function renderMessagesLog(a) {
        ...
        el.innerHTML =
            ...
            <div class="p-3 text-sm">
                ...
                .join("") ||
            '<div class="p-3 text-xs text-muted-foreground text-center">No messages</div>';
    }
    ...
    function handleDetailMoreAction(act) {
        ...
        if (act === "post-auction") {
            openModal($("post-auction-modal"));
            populatePostAuction(state.activeAuction);
        }
        if (act === "archive") {
    ...
    function addMockLot(a) {
        ...
        const mock = {
            id: "L" + genId(""),
            lot_num: num,
            listing: "LST-" + Math.floor(Math.random() * 9000 + 1000),
    ...
    function bindAucTabs() {
        document
            .querySelectorAll(".auc-tab-btn")
            .forEach((b) => (b.onclick = () => switchAucTab(b.dataset.tab)));
    }
    ...
    function renderConsoleLotsList(a) {
        ...
        el.querySelectorAll("[data-follow-lot]").forEach(
            (b) =>
                (b.onclick = () => {
                    const lot = a.lots.find(
                        (l) => l.id === b.dataset.followLot,
                    );
    ...
    function followLot(a, lot) {
        state.activeLot = lot;
        $("console-lot-title").textContent =
            `Lot ${lot.lot_num} — ${lot.vehicle}`;
        $("console-lot-meta").textContent =
            `Listing: ${lot.listing} · State: ${lot.state}`;
    ...
    function bindConsoleControls() {
        ...
        $("console-btn-pause-lot").onclick = () => {
            ...
            state.activeLot.state =
                state.activeLot.state === "Paused" ? "Live" : "Paused";
        ...
        $("console-btn-rerun-lot").onclick = () => {
            toast("Re-run scheduled", "info");
            logEvent("lot_rerun_created", { lotId: state.activeLot?.id });
        };
        $("console-btn-announce-lot").onclick = () =>
            toast("Announcement sent to participants", "info");
        $("console-btn-pause-all").onclick = () => {
            if (!state.activeAuction) return;
            state.activeAuction.status = "Paused";
            state.activeAuction.lots.forEach((l) => {
                if (l.state === "Live") l.state = "Paused";
            });
            toast("All lots paused");
            renderConsoleLotsList(state.activeAuction);
        };
        $("console-proxy-filter").onchange = () => {
            if (state.activeLot) renderConsoleBidFeed(state.activeLot);
        };
    }
    ...
    function openLotDetail(a, lot) {
        ...
        $("lot-detail-sub").textContent =
            `${lot.listing} · ${lot.vehicle} · ${lot.state}`;

        // overview
        if ($("lot-vehicle-summary"))
            $("lot-vehicle-summary").innerHTML = `
        ...
        if ($("lot-pricing-summary"))
            $("lot-pricing-summary").innerHTML = `
        ...
        if (feedEl)
            feedEl.innerHTML = lot.bids.length
                ? lot.bids
                      .map(
                          (b) => `
        ...
        // rules
        if ($("lot-rules-content"))
            $("lot-rules-content").innerHTML = `
        ...
        // tab buttons
        document
            .querySelectorAll(".lot-tab-btn")
            .forEach((b) => (b.onclick = () => switchLotTab(b.dataset.tab)));

        // action buttons
        $("lot-btn-extend").onclick = () => {
            toast(`Lot ${lot.lot_num} extended`, "info");
            logEvent("lot_extended", { lotId: lot.id });
        };
        $("lot-btn-withdraw").onclick = () => {
            lot.state = "Withdrawn";
            toast(`Lot ${lot.lot_num} withdrawn`);
            logEvent("lot_withdrawn", { lotId: lot.id });
        };
        $("lot-btn-rerun").onclick = () => {
            toast("Re-run scheduled", "info");
            logEvent("lot_rerun_created", { lotId: lot.id });
        };
        $("lot-btn-announce").onclick = () =>
            toast("Announcement sent", "info");
    ...
    function populatePostAuction(a) {
        ...
        if (!tbody) return;
        const ended = a.lots.filter(
            (l) => l.state === "Ended" || l.current_bid > 0,
        );
        if (!ended.length) {
            tbody.innerHTML = `<tr><td colspan="6" class="p-4 text-center text-xs text-muted-foreground">No ended lots yet</td></tr>`;
            return;
        }
        tbody.innerHTML = ended
            .map((l) => {
        ...
        tbody.querySelectorAll("[data-pa-act]").forEach(
            (b) =>
                (b.onclick = () => {
                    const lot = a.lots.find((l) => l.id === b.dataset.lotId);
                    const act = b.dataset.paAct;
                    if (act === "open-deal") {
                        logEvent("deal_opened_from_auction", { lotId: lot.id });
                        toast(`Deal opened for lot ${lot.lot_num}`);
                    }
                    if (act === "objection") {
                        toast(
                            `7-day objection window noted for lot ${lot.lot_num}`,
                            "info",
                        );
                    }
                    if (act === "rerun") {
                        logEvent("lot_rerun_created", { lotId: lot.id });
                        toast(`Re-run scheduled for lot ${lot.lot_num}`);
                    }
                    if (act === "offer-highest") {
                        logEvent("post_auction_offer_sent", { lotId: lot.id });
                        toast(
                            `Offer sent to highest bidder for lot ${lot.lot_num}`,
                        );
                    }
                    if (act === "switch-bin") {
                        if (lot) lot.bin_enabled = true;
                        toast(`Lot ${lot.lot_num} switched to BIN/Offer`);
                    }
                }),
        );
    }
    ...
    function openExchangeProposal(a, lot) {
        const ep = a.exchange_proposals.find(
            (e) => e.lot_id === lot.id && e.active,
        );
        ...
        if (ep) {
            ...
            $("ep-cash-diff").textContent = ep.cash_diff
                ? fmt(ep.cash_diff)
                : "Even swap";
    ...
    function bindExchangeProposalActions(a, lot, ep) {
        $("ep-btn-accept").onclick = () => {
            if (!ep) return;
            ep.active = false;
            logEvent("exchange_proposal_accepted", {
                lotId: lot.id,
                proposalId: ep.id,
            });
        ...
        $("ep-btn-counter").onclick = () =>
            toast("Counter-proposal (mock)", "info");
        $("ep-btn-submit").onclick = () => {
            const existingActive = a.exchange_proposals.find(
                (e) => e.lot_id === lot.id && e.active,
            );
            if (existingActive) {
                toast("Only 1 active proposal per listing allowed", "error");
                return;
            }
            const newEp = {
                id: "EP-" + genId(""),
                ...
                offered_by: "Me",
                offered_vehicle:
                    document.querySelector('[name="ep_vehicle"]')?.value || "—",
                cash_diff:
                    parseFloat(
                        document.querySelector('[name="ep_cash_diff"]')?.value,
                    ) || 0,
                expiry:
                    document.querySelector('[name="ep_expiry"]')?.value || "",
    ...
    function bindPublishConfirm() {
        $("confirm-publish-btn").onclick = () => {
            ...
            if (!a.lots.length) {
                toast("Cannot publish: no lots", "error");
                return;
            }
            a.status = "Published";
    ...
    function closeModal(el) {
        ...
        if (el.id === "live-console-modal" && state.liveConsoleInterval) {
            clearInterval(state.liveConsoleInterval);
            state.liveConsoleInterval = null;
        }
    }
    ...
    function bindModalClose() {
        document
            ...
            .forEach(
                (e) => (e.onclick = () => closeModal(e.closest(".fixed"))),
            );
        document
        ...
        document.addEventListener("click", (ev) => {
            if (
                !ev.target.closest("#bulk-actions-toggle") &&
                !ev.target.closest("#bulk-actions-menu")
            )
                $("bulk-actions-menu")?.classList.add("hidden");
            if (
                !ev.target.closest("#auc-more-toggle") &&
                !ev.target.closest("#auc-more-menu")
            )
                $("auc-more-menu")?.classList.add("hidden");
        });
    }
    ...
    function logEvent(event, data) {
        ...
        if (log) {
            ...
            if (log.firstChild?.classList?.contains("text-center"))
                log.innerHTML = "";
            log.prepend(div);
        }
    }

    function bindParticipantMode() {
        const radios = document.querySelectorAll(
            'input[name="participant_mode"]',
        );
        const vendorWrap = document.getElementById("vendor-select-wrap");
        const setWrap = document.getElementById("participant-set-select");

        function updateView() {
            const selected = document.querySelector(
                'input[name="participant_mode"]:checked',
            )?.value;

            vendorWrap?.classList.add("hidden");
            setWrap?.classList.add("hidden");

            if (selected === "invite") {
                vendorWrap?.classList.remove("hidden");
            }
