...
$vehicleChecksData = [
    ...
    'mot-advisories' => [
        ...
        'advisories' => $listing['mot_advisories'] ?? ['Nearside front tyre wearing close to the legal limit.'],
    ...
    {{-- ===== BODY: TABS + RIGHT PANEL ===== --}}
                    ...
                    <div class="p-4 space-y-4">
                     ...
                     timestamp, delta vs Guide/Reserve, and buttons: View history / Pull latest valuation / Apply to pricing --}}
                        <div data-tab-pane="overview" class="space-y-4">
                            ...
                            <div class="card border border-border rounded-xl p-4" id="valuation-card">
                                ...
                                <div class="flex flex-wrap items-end gap-6">
                                    ...
                                    <div>
                                        ...
                                        <div class="font-medium text-sm" id="ov-valuation-source">
                                            {{ $listing['valuation_source'] ?? 'Internal (Carsmart)' }}</div>
                                    </div>
                                    <div>
                                        <div class="text-xs text-muted-foreground mb-1">Last fetched</div>
                                        <div class="font-medium text-sm" id="ov-valuation-timestamp">
                                            {{ $listing['valuation_timestamp'] ?? '2026-05-31 09:03' }}</div>
                                    </div>
                                    <div>
                                        ...
                                        @endphp
                                        <span
                                            class="kt-badge {{ $delta >= 0 ? 'kt-badge-success' : 'kt-badge-danger' }} text-xs"
                                            id="ov-delta-guide">
                                    ...
                                    <div>
                                        ...
                                        @endphp
                                        <span
                                            class="kt-badge {{ $deltaR >= 0 ? 'kt-badge-success' : 'kt-badge-danger' }} text-xs"
                                            id="ov-delta-reserve">
                                ...
                                <div class="flex gap-2 mt-4">
                                    <button class="kt-btn kt-btn-xs kt-btn-outline"
                                        onclick="document.querySelector('.detail-tab[data-tab=\'valuations\']')?.click()">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                                @foreach ([['Vehicle', $listing['vehicle'] ?? '—'], ['VRM', $listing['vrm'] ?? '—'], ['Mileage', number_format($listing['mileage'] ?? 0) . ' mi'], ['State', $listing['state'] ?? '—'], ['Sale Type', $listing['sale_type'] ?? 'CST1'], ['QA Status', $listing['qa'] ?? '—'], ['KYC', $listing['kyc_status'] ?? 'Required'], ['Owner', $listing['owner'] ?? '—']] as [$l, $v])
                            ...
                            @if (($listing['missing_items'] ?? 0) > 0)
                                <div
                                    class="rounded-xl border border-warning/30 bg-warning/10 px-4 py-3 text-sm text-warning-foreground">
                                    <i class="ki-filled ki-information-4 mr-1"></i>
                                    {{ $listing['missing_items'] }} item(s) missing before this listing can move to
                                    Publication Queue. Check QA tab.
                                </div>
                            @endif
                     ...
                     Vehicle Details / Vehicle History & Checks (HPI, Previous Owners, MOT Advisories, Exterior Grading) --}}
                        <div data-tab-pane="vehicle" class="hidden space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label class="text-xs text-muted-foreground mb-1 block">Vehicle Smoking Status
                                        *</label>
                                    <select class="kt-input w-full">
                            ...
                            <div>
                                <label class="text-xs text-muted-foreground mb-1 block">If Advertised, Where?
                                    (Name)</label>
                            ...
                            <div class="border-t border-border pt-4">
                                ...
                                <div class="flex flex-wrap gap-2" id="vehicle-features-list">
                                    @forelse (($listing['features'] ?? ['Navigation', 'Heated Seats', 'Parking Sensors', 'Cruise Control']) as $feature)
                                        <span
                                            class="kt-badge kt-badge-outline text-xs px-3 py-1.5 flex items-center gap-1.5">
                            ...
                            <div class="border-t border-border pt-4">
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                                    @foreach ([['Exterior Color', $listing['exterior_colour'] ?? ($listing['vehicle_colour'] ?? '—')], ['Seats Material', $listing['seats_material'] ?? '—'], ['Body Type', $listing['vehicle_body_type'] ?? '—'], ['Gearbox', $listing['gearbox'] ?? '—'], ['Fuel Type', $listing['vehicle_fuel_type'] ?? '—'], ['Engine Size', $listing['engine_size'] ?? '—']] as [$l, $v])
                                        <div class="card border border-border p-3 rounded-xl">
                                            <div class="text-xs text-muted-foreground">{{ $l }}</div>
                                            <div class="font-medium mt-0.5 text-sm">{{ $v }}</div>
                                        </div>
                            ...
                            <div class="border-t border-border pt-4">
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                                    ...
                                    <button type="button"
                                        class="card border border-border rounded-xl p-3 text-left hover:bg-muted/5 transition"
                                        data-history-check="hpi">
                                        ...
                                        <div class="font-medium text-sm">{{ $listing['hpi_summary'] ?? 'All Clear' }}
                                        </div>
                                    </button>
                                    ...
                                    <div class="card border border-border p-3 rounded-xl">
                                        ...
                                        <div class="font-medium mt-0.5 text-sm">{{ $listing['first_registered'] ?? '—' }}
                                        </div>
                                    </div>
                                    <div class="card border border-border p-3 rounded-xl">
                                        <div class="text-xs text-muted-foreground">Keeper Start Date</div>
                                        <div class="font-medium mt-0.5 text-sm">{{ $listing['keeper_start_date'] ?? '—' }}
                                        </div>
                                    </div>
                                    ...
                                    <div class="card border border-border p-3 rounded-xl">
                                        ...
                                        <div class="font-medium mt-0.5 text-sm">
                                            {{ $listing['on_finance'] ?? false ? 'Yes' : 'No' }}</div>
                                    </div>
                                    <div class="card border border-border p-3 rounded-xl">
                                        <div class="text-xs text-muted-foreground">Private Plate</div>
                                        <div class="font-medium mt-0.5 text-sm">
                                            {{ $listing['private_plate'] ?? false ? 'Yes' : 'No' }}</div>
                                    </div>
                                    <div class="card border border-border p-3 rounded-xl">
                                        <div class="text-xs text-muted-foreground">Seller Keeping Plate</div>
                                        <div class="font-medium mt-0.5 text-sm">
                                            {{ $listing['seller_keeping_plate'] ?? false ? 'Yes' : 'No' }}</div>
                                    </div>
                                    <div class="card border border-border p-3 rounded-xl">
                                        <div class="text-xs text-muted-foreground">Original Plate</div>
                                        <div class="font-medium mt-0.5 text-sm">{{ $listing['original_plate'] ?? '—' }}
                                        </div>
                                    </div>
                            ...
                            <div class="border-t border-border pt-4">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label class="text-xs text-muted-foreground mb-1 block">Vehicle Seat Type
                                            (Notes)</label>
                                        <textarea class="kt-input w-full" rows="3"
                                    ...
                                    <div>
                                        <label class="text-xs text-muted-foreground mb-1 block">Tool Pack Status
                                            (Notes)</label>
                                    ...
                                    <div>
                                        <label class="text-xs text-muted-foreground mb-1 block">Charging Cables
                                            (Notes)</label>
                            ...
                            <div class="border-t border-border pt-4">
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <button type="button" class="kt-btn kt-btn-xs kt-btn-outline"
                                        data-history-check="mot-advisories">
                                ...
                                <div class="mt-3">
                                    ...
                                    <textarea class="kt-input w-full text-xs" rows="2"
                                        placeholder="e.g., Nearside front tyre wearing close to legal limit, etc."></textarea>
                                </div>
                            </div>
                            ...
                            <div class="border-t border-border pt-4">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label class="text-xs text-muted-foreground mb-1 block">Mechanical & Electrical
                                            Issues? *</label>
                                        <select class="kt-input w-full">
                                            <option value="no">No</option>
                                            <option value="yes">Yes</option>
                                    ...
                                    <div>
                                        <label class="text-xs text-muted-foreground mb-1 block">Issue Details
                                            (Notes)</label>
                            ...
                            <div class="border-t border-border pt-4">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    @foreach (['Front Drivers Side', 'Front Passengers Side', 'Rear Passenger Side', 'Rear Drivers Side', 'Front Seats', 'Rear Seats', 'Dashboard', 'Boot', 'Front Drivers Wheel/Tyre', 'Front Passengers Wheel/Tyre', 'Rear Drivers Wheel/Tyre', 'Rear Passengers Wheel/Tyre'] as $area)
                                        <div class="card border border-border p-3 rounded-xl">
                                            <div class="font-medium text-sm mb-2">{{ $area }}</div>
                                            <div
                                                class="border-2 border-dashed border-border rounded-lg p-3 text-center cursor-pointer hover:bg-muted/5 mb-2">
                                                <i
                                                    class="ki-filled ki-add-image text-lg text-muted-foreground mb-1 block"></i>
                                        ...
                                        <div class="card border border-border p-4 rounded-xl">
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div class="space-y-3">
                                                    ...
                                                    <div>
                                                        <label
                                                            class="text-xs text-muted-foreground block mb-2">Photo</label>
                                                        <div
                                                            class="border-2 border-dashed border-border rounded-lg p-4 text-center cursor-pointer hover:bg-muted/5 transition">
                                                            <i
                                                ...
                                                <div class="space-y-2">
                                                    ...
                                                    <div>
                                                        <label class="text-xs text-muted-foreground mb-1 block">Vehicle
                                                            Side (Location)</label>
                     ...
                     (folded in from the old "About You" + part of "Documents") --}}
                        <div data-tab-pane="seller" class="hidden space-y-4">
                            ...
                            <div class="border-t border-border pt-4">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label class="text-xs text-muted-foreground mb-1 block">Plate Status
                                            (Notes)</label>
                                    ...
                                    <div>
                                        <label class="text-xs text-muted-foreground mb-1 block">Finance Details
                                            (Notes)</label>
                        ...
                        <div data-tab-pane="media" class="hidden space-y-4">
                            ...
                            <div
                                class="flex items-center justify-between rounded-lg bg-muted/5 border border-border p-3 text-xs">
                        ...
                        <div data-tab-pane="documents" class="hidden space-y-4">
                            ...
                            <div class="grid grid-cols-1 gap-3">
                                @foreach ([['V5C Logbook', 'v5c-logbook'], ['V5C Owner Document', 'v5c-owner'], ['V5C Print Vehicle Address', 'v5c-address'], ['Proof Of Purchase', 'proof-purchase'], ['Service Records', 'service-records'], ['Manuals', 'manuals'], ['MOT Certificate', 'mot-certificate']] as [$label, $id])
                                    <div class="card border border-border p-4 rounded-xl">
                                        <div class="flex items-center justify-between mb-3">
                        ...
                        <div data-tab-pane="pricing" class="hidden space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input class="kt-input w-full" type="number" value="{{ $listing['guide'] ?? '' }}"
                                        id="pricing-guide-input">
                                </div>
                                <div>
                                    <label class="text-xs text-muted-foreground mb-1 block">Reserve £</label>
                                    <input class="kt-input w-full" type="number"
                                        value="{{ $listing['reserve'] ?? '' }}" id="pricing-reserve-input">
                                </div>
                                <div>
                                    <label class="text-xs text-muted-foreground mb-1 block">BIN Price £</label>
                                    <input class="kt-input w-full" type="number"
                                        value="{{ $listing['bin_price'] ?? '' }}" placeholder="Leave blank if BIN off">
                                </div>
                                <div>
                                ...
                                <table class="w-full text-xs">
                                    ...
                                    <tbody class="divide-y divide-border" id="valuation-log-body">
                                        <tr>
                                            ...
                                            <td class="p-2.5"><span
                                                    class="kt-badge kt-badge-success text-xs">+£200</span></td>
                                            <td class="p-2.5 text-muted-foreground">Auto-fetched</td>
                                            <td class="p-2.5 text-center"><i class="ki-filled ki-check text-success"></i>
                                            </td>
                                            <td class="p-2.5 text-center"><i class="ki-filled ki-check text-success"></i>
                                            </td>
                                            <td class="p-2.5 text-right whitespace-nowrap">
                                                <button class="kt-btn kt-btn-xs kt-btn-outline"
                                                    data-listing-action="apply-pricing">Apply</button>
                                                <button class="kt-btn kt-btn-xs kt-btn-outline text-danger"
                                                    data-listing-action="remove-valuation">Remove</button>
                                            </td>
                                        </tr>
                                        <tr>
                                            ...
                                            <td class="p-2.5"><span
                                                    class="kt-badge kt-badge-success text-xs">+£4,200</span></td>
                                        ...
                                        <tr>
                                            ...
                                            <td class="p-2.5"><span class="kt-badge kt-badge-danger text-xs">−£200</span>
                                            </td>
            ...
            {{-- ===== APPLY TO PRICING MODAL (delta preview + confirmation) ===== --}}
                ...
                <div class="bg-background border border-border rounded-xl w-full max-w-lg">
                    ...
                    <div class="flex items-center justify-between p-4 border-b border-border">
                        ...
                        <button class="kt-btn kt-btn-sm kt-btn-icon kt-btn-outline"
                            data-modal-close="apply-pricing-modal">
                    ...
                    <div class="p-4 space-y-4">
                        <p class="text-sm text-muted-foreground">
                            Review the change before it's applied to this listing's pricing. This action is logged to
                            Activity & History.
                        </p>
                            ...
                            <table class="w-full text-sm min-w-[480px]">
                                ...
                                <tbody class="divide-y divide-border">
                                    <tr>
                                        ...
                                        <td class="p-2.5 text-muted-foreground" id="apply-guide-current">
                                            £{{ number_format($listing['guide'] ?? 14250) }}</td>
                                        <td class="p-2.5 font-semibold" id="apply-guide-new">
                                            £{{ number_format($listing['valuation'] ?? 14200) }}</td>
                                    ...
                                    <tr>
                                        ...
                                        <td class="p-2.5 text-muted-foreground" id="apply-reserve-current">
                                            £{{ number_format($listing['reserve'] ?? 13950) }}</td>
                    ...
                    <div class="flex items-center justify-end gap-2 p-4 border-t border-border">
                        <button class="kt-btn kt-btn-sm kt-btn-outline"
                            data-modal-close="apply-pricing-modal">Cancel</button>
            ...
            <div id="pull-valuation-modal" class="hidden fixed inset-0 z-50 items-center justify-center bg-black/50 p-4">
                <div class="bg-background border border-border rounded-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
                    ...
                    <div class="p-4 pb-0" id="pull-valuation-fetch-state">
                        <div class="hidden rounded-lg border border-info/30 bg-info/10 px-3 py-2 text-xs text-info flex items-center gap-2"
                            data-fetch-state="in-progress">
                            <i class="ki-filled ki-loading animate-spin"></i> Fetching latest valuations from all
                            providers…
                        </div>
                        <div class="hidden rounded-lg border border-success/30 bg-success/10 px-3 py-2 text-xs text-success flex items-center gap-2"
                            data-fetch-state="succeeded">
                            <i class="ki-filled ki-check-circle"></i> All valuations refreshed successfully.
                        </div>
                        <div class="hidden rounded-lg border border-danger/30 bg-danger/10 px-3 py-2 text-xs text-danger flex items-center justify-between gap-2"
                            data-fetch-state="failed">
                            <span><i class="ki-filled ki-cross-circle mr-1"></i> Some providers failed to respond (rate
                                limited). Showing last known values.</span>
                ...
                <div class="bg-background border border-border rounded-xl w-full max-w-3xl max-h-[85vh] overflow-y-auto">
                    ...
                    <div class="p-4 pb-0">
                        <div class="rounded-xl border border-primary/30 bg-primary/5 p-4">
                            <label class="text-xs font-semibold text-foreground mb-1 block">Set as Official Valuation
                                *</label>
                            <p class="text-xs text-muted-foreground mb-2">
                                Choose which figure below becomes this listing's official valuation on save. It will update
                                the Overview card, recalculate deltas vs Guide/Reserve, and add an entry to the Valuations
                                log.
                            </p>
                            <select class="kt-input w-full" id="official-valuation-select">
                                ...
                                <optgroup label="AutoTrader">
                                    <option value="autotrader_recommended_price">AutoTrader Recommended Selling Price
                                    </option>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                ...
                                <div>
                                    <label class="text-xs text-muted-foreground mb-1 block">Trade In Valuation Poor HIGH
                                        £</label>
                                    <input class="kt-input w-full text-sm" type="number"
                                        name="hpi_trade_in_poor_high" placeholder="15600">
                                </div>
                                <div>
                                ...
                                <div>
                                    <label class="text-xs text-muted-foreground mb-1 block">Trade In Valuation Good HIGH
                                        £</label>
                                    <input class="kt-input w-full text-sm" type="number"
                                        name="hpi_trade_in_good_high" placeholder="16830">
                                </div>
                                <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                ...
                                <div>
                                    <label class="text-xs text-muted-foreground mb-1 block">More Than Part Exchange
                                        £</label>
                                    <input class="kt-input w-full text-sm" type="number"
                                        name="autotrader_more_than_pex" placeholder="3500">
                                </div>
                                <div>
         ...
         Restyled to match the theme-aware pattern used by the other modals above:
         bg-background / border-border / text-muted-foreground so it adapts to light & dark mode. --}}
            <div id="vehicle-check-modal" class="hidden fixed inset-0 z-50 items-center justify-center bg-black/50 p-4">
                <div class="bg-background border border-border rounded-xl w-full max-w-lg max-h-[85vh] overflow-y-auto">

                    <div
                        class="flex items-center justify-between p-4 border-b border-border sticky top-0 bg-background z-10">
                        <h3 class="font-semibold text-base" id="vc-modal-title">Vehicle Check</h3>
                        <button class="kt-btn kt-btn-sm kt-btn-icon kt-btn-outline"
                            data-modal-close="vehicle-check-modal">
                            <i class="ki-filled ki-cross"></i>
                        </button>
                    </div>
                    ...
                    <div class="p-4 space-y-4">
                        ...
                        <div class="flex items-center gap-2 flex-wrap">
                            <span
                                class="flex items-center rounded-md overflow-hidden text-sm font-bold border border-border">
                                <span class="bg-primary w-2 h-full block"></span>
                                <span class="px-3 py-1.5 tracking-wide bg-muted/10"
                                    id="vc-modal-vrm">{{ $listing['vrm'] ?? 'AB19 CDE' }}</span>
                            </span>
                            <span class="kt-badge kt-badge-outline text-xs" id="vc-modal-subcount"></span>
                        </div>

                        <div class="flex items-center gap-3">
                            <div
                                class="w-11 h-11 rounded-full bg-muted/10 border border-border flex items-center justify-center shrink-0">
                                <i class="ki-filled ki-car text-xl text-muted-foreground"></i>
                            </div>
                            <div class="min-w-0">
                                <div class="font-semibold text-sm leading-tight truncate" id="vc-modal-vehicle-name">
                                    {{ $listing['vehicle'] ?? 'Vehicle' }}</div>
                                <div class="text-xs text-muted-foreground mt-0.5" id="vc-modal-vehicle-meta">
                                    {{ $vcVehicleMeta ?: $listing['vehicle'] ?? '' }}</div>
                            </div>
                        </div>

                        <div class="rounded-xl border border-border bg-muted/5 p-4" id="vc-modal-body">
                            {{-- populated dynamically per check type --}}
                        </div>

                        <p class="text-xs text-muted-foreground" id="vc-modal-footnote"></p>
                    </div>
                </div>
            </div>
            ...
            <script>
                (function() {
                    ...
                    ['pull-valuation-modal', 'add-valuation-modal', 'apply-pricing-modal', 'vehicle-check-modal'].forEach(
                    id => {
                    ...
                    function simulateFetch() {
                        ...
                        const banners = {};
                        states.forEach(s => {
                            banners[s] = document.querySelector(
                            `#pull-valuation-fetch-state [data-fetch-state="${s}"]`);
                    ...
                    document.querySelector('#save-add-valuation-btn')?.addEventListener('click', () => {
                        ...
                        if (!chosenField || chosenAmount === null || chosenAmount === undefined || isNaN(
                            chosenAmount)) {
                ...
                `;
                            ...
                            row.querySelector('[data-listing-action="remove-valuation"]')?.addEventListener('click', (
                                e) => {
                                    if (!confirm('Remove this valuation record?')) return;
                                    e.target.closest('tr')?.remove();
                                });
                            row.querySelector('[data-listing-action="apply-pricing"]')?.addEventListener('click', () =>
                                openModal('apply-pricing-modal'));
                        }
                    ...
                    // Rendered with theme-aware utility classes (text-muted-foreground, divide-border, bg-muted/5, text-success)
                    // so the modal content follows light/dark mode like the rest of the app.
                    ...
                    `<li class="flex items-start gap-2"><i class="ki-filled ki-check-circle text-success mt-0.5"></i><span>${i}</span></li>`
                ).join('')}</ul>`;
                            footnote.textContent =
                                `Vehicle history check completed on ${data.checked_at} using data from ${data.source}. Please note that all warranties are disclaimed — we recommend carrying out your own checks to ensure you're fully informed.`;
                        } else if (type === 'owners') {
                            subcount.textContent = `${data.count} owner${data.count === 1 ? '' : 's'}`;
                            body.innerHTML = `<table class="w-full text-sm">
                    <thead><tr class="text-left text-muted-foreground text-xs">
                    ...
                    ${data.advisories.map(a => `<div class="bg-muted/10 border border-border rounded-lg px-3 py-2">${a}</div>`).join('')}
                </div>`;
                    ...
                    <tbody class="divide-y divide-border">
                        ${data.rows.map(r => `<tr><td class="py-2 pr-3 text-muted-foreground">${r.label}</td>${data.columns.map(c => `<td class="py-2 pr-3">${r[c] ?? '—'}</td>`).join('')}</tr>`).join('')}
                    </tbody>
                </table></div>`;
                        }
                    }
