...
<div class="kt-container-fixed py-6 flex flex-col gap-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        {{-- Deals pending --}}
        <a href="{{ route('payments.transactions') }}" class="kt-card hover:border-primary/40 transition-colors">
            <div class="kt-card-content p-5 flex flex-col gap-3">
                <div class="flex items-center justify-between">
    ...
    <div class="grid lg:grid-cols-3 gap-4">
        ...
        <div class="lg:col-span-2 kt-card flex flex-col">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div class="kt-menu" data-kt-menu="true">
                        <div class="kt-menu-item"
                            ...
                            <button class="kt-menu-toggle kt-btn kt-btn-ghost kt-btn-sm flex items-center gap-1.5 text-sm">
                                All types
            ...
            <div class="kt-card-content p-0">
                @php
                $workQueue = [
                    ['type' => 'Listing',    'label' => 'Listings needing quality assurance', 'count' => $queue['qa_listings'] ?? 0,       'route' => 'listings.index',       'icon' => 'ki-row-horizontal', 'color' => 'primary'],
                    ['type' => 'Valuation',  'label' => 'Valuations waiting',                  'count' => $queue['valuations'] ?? 0,         'route' => 'listings.index',       'icon' => 'ki-dollar',         'color' => 'warning'],
                    ['type' => 'Objection',  'label' => 'Seller objections',                   'count' => $queue['objections'] ?? 0,         'route' => 'disputes.open',        'icon' => 'ki-message-edit',   'color' => 'danger'],
                    ['type' => 'Dispute',    'label' => 'Disputes',                            'count' => $queue['disputes'] ?? 0,           'route' => 'disputes.open',        'icon' => 'ki-shield-cross',   'color' => 'danger'],
                ];
                @endphp
                    ...
                    <a href="{{ route($item['route']) }}"
                       class="flex items-center justify-between gap-3 px-5 py-4 border-b border-border last:border-0 hover:bg-accent/40 transition-colors group">
                        <div class="flex items-center gap-3">
                            <span class="flex items-center justify-center size-8 rounded-lg bg-{{ $item['color'] }}/10 shrink-0">
                        ...
                        <div class="flex items-center gap-3">
                            <span class="kt-badge kt-badge-{{ $item['color'] }} kt-badge-outline text-xs font-semibold px-2.5 py-1">
                                {{ $item['count'] }}
                            </span>
                            <i class="ki-filled ki-right text-muted-foreground text-xs group-hover:text-primary transition-colors"></i>
                        </div>
                    </a>
                @empty
                    <div class="flex flex-col items-center justify-center py-12 px-5 text-center">
                        ...
                        <p class="text-sm text-muted-foreground">Nothing urgent right now. You can open Listings or Auctions from the menu.</p>
                    </div>
                @endforelse
        ...
        <div class="kt-card flex flex-col">
            ...
            <div class="kt-card-content p-0 grow overflow-y-auto">
                ...
                @php
                $alerts = $alerts ?? [
                    ['type' => 'identity',  'label' => 'Failed identity verification',    'count' => 2, 'icon' => 'ki-user-tick',    'route' => 'customers.index'],
                    ['type' => 'business',  'label' => 'Failed business verification',    'count' => 1, 'icon' => 'ki-shop',         'route' => 'vendors.index'],
                    ['type' => 'reserve',   'label' => 'Reserve price not set',           'count' => 4, 'icon' => 'ki-dollar',       'route' => 'auctions.index'],
                    ['type' => 'photos',    'label' => 'Missing photos',                  'count' => 7, 'icon' => 'ki-picture',      'route' => 'listings.index'],
                ];
                @endphp
        ...
        <div class="kt-card-content p-0">
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-border bg-muted/40">
                                <th class="text-start px-5 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Auction</th>
                                <th class="text-start px-5 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Lots</th>
                                <th class="text-start px-5 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Status</th>
                                <th class="text-start px-5 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Closes</th>
                                <th class="text-end px-5 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($todaysAuctions as $auction)
                                <tr class="border-b border-border last:border-0 hover:bg-accent/30 transition-colors">
                                    ...
                                    <td class="px-5 py-3.5">
                                        @if ($auction['status'] === 'live')
                                            <span class="kt-badge kt-badge-success text-xs font-medium">
                                                <span class="size-1.5 rounded-full bg-success me-1.5 animate-pulse inline-block"></span>
                                                Live
                                            </span>
                                        @elseif ($auction['status'] === 'closing_soon')
                                            <span class="kt-badge kt-badge-warning text-xs font-medium">Closing soon</span>
                                        @else
                                    ...
                                    <td class="px-5 py-3.5 text-end">
                                        <a href="#" class="kt-btn kt-btn-ghost kt-btn-sm kt-btn-icon" title="Open bid feed">
            ...
            @else
                <div class="flex flex-col items-center justify-center py-12 px-5 text-center">
                    ...
                    <p class="text-xs text-muted-foreground">Nothing urgent right now. You can open Listings or Auctions from the menu.</p>
                </div>
            @endif
